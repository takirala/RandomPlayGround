...
public class ConcurrentSearcherListOld<T> {
    ...
    Semaphore searchAcquirer = new Semaphore(1);
    Semaphore insertAcquirer = new Semaphore(1);


    AtomicInteger concurrenSearches = new AtomicInteger(0);
    ...
    public ConcurrentSearcherListOld() {
        first = null;
    }
    ...
    private void start_insert() throws InterruptedException {
        insertAcquirer.acquire();
        if (concurrentInserts.incrementAndGet() == 1) {
            insertKey.acquire();
        }
        insertAcquirer.release();
        insertSem.acquire();
    }
    ...
    private void start_search() throws InterruptedException {
        searchAcquirer.acquire();
        if (concurrenSearches.incrementAndGet() == 1) {
            searchKey.acquire();
        }
        searchAcquirer.release();
    }
