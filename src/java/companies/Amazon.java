...
public class Amazon
{
    ...
    {
        System.out.println(doOverLap(new Point(-3D, 0.0D), new Point(3D, 4D), new Point(0.0D, -2D), new Point(9D, -1D)));
    }
    ...
    private static Collection kClosest(Collection points, int k)
    {
        ...
        for(Iterator iterator = points.iterator(); iterator.hasNext();)
        {
            ...
            if(((Point)pq.peek()).distance > p.distance)
            {
                pq.poll();
                pq.offer(p);
            }
