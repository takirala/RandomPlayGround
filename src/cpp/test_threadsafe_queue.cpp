...
//	std::cout << "Printing sorted contents of queue" << std::endl << queue << std::endl;
	...
	int i = 0;
	for (auto iter = backing_queue.begin(); iter != backing_queue.end();
			++iter, ++i) {
		if (*iter != i) {
 ...
 * 	number of activer adder threads is 0.  Remover threads put the values they
...
int test_queue_2(int num_adders, int num_removers, int vals_per_adder) {
	...
	std::vector<std::list<int>> removed_vals(num_removers);
							...
							for(;;) {
								try {
									for(;;) {
										int my_val;
										queue.front(my_val);
										removed_vals[my_t].push_back(my_val);
									}
								}
